...
class _nbr:
    ...
    def _if_pos_neg(ss, obj, silent=False):
    ...
    def __le__(ss, obj):
        return ss.value <= ss._if_pos_neg(obj)
    def __eq__(ss, obj):
        return ss.value == ss._if_pos_neg(obj, True)
    def __ne__(ss, obj):
        return ss.value != ss._if_pos_neg(obj, True)
    def __ge__(ss, obj):
        return ss.value >= ss._if_pos_neg(obj)
...
class nbr:
    def __init__(ss, nbr_):
        make_id(ss)

        if isinstance(nbr_, (neg, pos, nbr)):
            ss.value = abs(nbr_.value)
        elif isinstance(nbr_, py_str):
            ss.value = abs(mk_nbr(nbr_, return_value=True))
        else:
            ss.value = abs(int(nbr_))
